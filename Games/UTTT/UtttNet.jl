...
function resconv2heads(n_filter,n_tower)
     ...
     return resconv2heads(Chain(Conv((3,3),6=>n_filter,stride=1,pad=1),BatchNorm(n_filter,relu)),[resnet(n_filter) for k in 1:n_tower]
     ,Chain(Conv((1,1),n_filter=>3,pad=0,stride=1),BatchNorm(3,relu),x->reshape(x,:,size(x,4)),Dense(243,81)),
     Chain(Conv((1,1),n_filter=>3,pad=0,stride=1),BatchNorm(3,relu), x->reshape(x,:,size(x,4)),Dense(243,1,tanh)))|>gpu
end
